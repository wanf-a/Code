...
def train_model(model, data, config, name):
    ...
    print(f"\n  训练: {name} | 参数量: {n_params:,}", flush=True)
    ...
    for epoch in range(config["max_epochs"]):
        ...
        for bi, (xb, yb) in enumerate(train_loader):
            ...
            if bi == 0 or (bi + 1) % max(1, n_batches // 4) == 0:
                print(f"    Epoch {epoch+1}/{config['max_epochs']} "
                      f"batch {bi+1}/{n_batches} | loss: {loss.item():.6f}", flush=True)
        ...
        if epoch < 3 or epoch % 5 == 0 or improved:
            ...
            print(f"    Epoch {epoch+1:3d}/{config['max_epochs']} | "
                  ...
                  f"LR: {lr:.1e} | {time.time()-t0:.0f}s{marker}", flush=True)
...
def rolling_week_predict(model, feat_scaled, input_len, week_starts, device):
    ...
    for i, start in enumerate(week_starts):
        for t in range(672):
            ...
            with torch.no_grad():
                all_pred[i, t, :] = model(x).cpu().numpy()[0]
...
def main():
    ...
    for wi in plot_indices:
        ...
        for d in range(1, 7):
            ax.axvline(d * 24, color='gray', ls='--', alpha=0.3)
        ax.set_title(f'TCN周预测 (样本#{wi})', fontsize=14)
        ax.set_xlabel('小时')
        ax.set_ylabel('电价 (元/MWh)')
    ...
    for model_name, model in trained_models.items():
        torch.save(model.state_dict(), os.path.join(root, "model_weights.pt"))
