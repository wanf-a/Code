...
def main():
    ...
    print(f"  模型: TCN-QR, Mamba-QR, NLinear-QR + Ensemble")
    ...
    for wi in plot_indices:
        ...
        if "Ensemble" in week_preds:
            ...
            ax.plot(hours, p50, label='集成',
                    color=COLORS["Ensemble"], lw=1.5, ls='--')
        for d in range(1, 7):
            ax.axvline(d * 24, color='gray', ls='--', alpha=0.3)
        ax.set_title(f'全模型周预测对比 (样本#{wi})', fontsize=14)
        ax.set_xlabel('小时')
        ax.set_ylabel('电价 (元/MWh)')
    ...
    for mn in list(model_names) + ["Ensemble"]:
        if mn not in all_results or "direct" not in all_results[mn]:
            continue
        r = all_results[mn]["direct"]
        label = f"{mn}-QR" if mn != "Ensemble" else "集成最优"
        print(f"  {label:12s} | {r['MAPE_150']:>10.4f} | "
