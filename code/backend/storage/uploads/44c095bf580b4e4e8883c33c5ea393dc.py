...
class TCN_QR(nn.Module):
    def __init__(self, input_size, nq=NQ,
                 channels=[64, 64, 64], kernel_size=5, dropout=0.2):
        super().__init__()
        layers = []
        in_ch = input_size
...
def train_model(model, data, config, name):
    ...
    for epoch in range(config["max_epochs"]):
        ...
        if val_loss < best_val - 1e-6:
            ...
            patience_counter += 1
            if epoch % 5 == 0 or patience_counter >= config["patience"] - 1:
                print(f"    Epoch {epoch+1:3d} | "
                      f"Train: {train_loss:.6f} Val: {val_loss:.6f} | "
                      f"LR: {lr:.1e} | {time.time()-t0:.0f}s "
...
def parse_args():
    ...
    parser.add_argument("--data", type=str,
        default="广东电价数据.csv",
        help="数据文件路径")
    parser.add_argument("--input_len", type=int, default=672)
...
def main():
    ...
    data = prepare_data(feat, target, args.input_len, stride=1)
    tsc = data["tgt_scaler"]
    ...
    model_factories = {
        "TCN": lambda: TCN_QR(data["input_size"]),
    ...
    print(f"\n{'='*70}")
    print(f"  集成概率预测 (测试集 Direct)")
    print(f"{'='*70}")

    qi50 = QUANTILES.index(0.5)
    model_names = list(trained_models.keys())

    best_weights_direct, best_mape_direct = None, 1.0
    for w0 in np.arange(0.1, 0.9, 0.1):
        for w1 in np.arange(0.1, 1.0 - w0, 0.1):
            w2 = round(1.0 - w0 - w1, 1)
            if w2 < 0.05:
                continue
            weights = [w0, w1, w2]
            ens = sum(w * test_preds[mn]
                      for w, mn in zip(weights, model_names))
            p50 = ens[:, qi50]
            p50_inv = tsc.inverse_transform(
                p50.reshape(-1, 1)).flatten()
            y_te_inv_tmp = tsc.inverse_transform(
                data["y_te"].reshape(-1, 1)).flatten()
            m = mape_clipped(y_te_inv_tmp, p50_inv, 150)
            if m < best_mape_direct:
                best_mape_direct = m
                best_weights_direct = {
                    mn: w for mn, w in zip(model_names, weights)
                }

    if best_weights_direct:
        ens_pred_direct = sum(w * test_preds[mn]
                              for mn, w in best_weights_direct.items())
        test_preds["Ensemble"] = ens_pred_direct
        ens_metrics_direct, _, _ = compute_all_metrics(data["y_te"], ens_pred_direct, tsc)
        all_results["Ensemble"] = {"direct": ens_metrics_direct, "weights": best_weights_direct}

        weight_str_direct = ", ".join(
            [f"{mn}={w:.1f}" for mn, w in best_weights_direct.items()])
        print(f"  最优权重: {weight_str_direct}")
        print(f"  集成 MAPE(150) = {ens_metrics_direct['MAPE_150']:.4f}")
        print(f"  集成 RMSE = {ens_metrics_direct['RMSE']:.2f} | R² = {ens_metrics_direct['R2']:.4f}")
        print(f"  集成 PICP90 = {ens_metrics_direct['PICP_90']:.3f} | "
              f"PICP95 = {ens_metrics_direct['PICP_95']:.3f}")
    ...
    for i in range(n_te):
        ...
        if "Ensemble" in test_preds:
            pq = test_preds["Ensemble"][i, :]
            pq_inv = tsc.inverse_transform(
                pq.reshape(-1, 1)).flatten()
            for qi, q in enumerate(QUANTILES):
                row[f"Ensemble_q{q}"] = pq_inv[qi]
        rows_te.append(row)
    ...
    best_weights, best_mape = None, 1.0
    for w0 in np.arange(0.1, 0.9, 0.1):
        for w1 in np.arange(0.1, 1.0 - w0, 0.1):
            w2 = round(1.0 - w0 - w1, 1)
            if w2 < 0.05:
                continue
            weights = [w0, w1, w2]
            ens = sum(w * week_preds[mn]
                      for w, mn in zip(weights, model_names))
            p50 = ens[:, :, qi50]
            p50_inv = tsc.inverse_transform(
                p50.reshape(-1, 1)).reshape(n_weeks, 672)
            wt_inv = tsc.inverse_transform(
                week_true.reshape(-1, 1)).reshape(n_weeks, 672)
            m = mape_clipped(wt_inv, p50_inv, 150)
            if m < best_mape:
                best_mape = m
                best_weights = {
                    mn: w for mn, w in zip(model_names, weights)
    ...
    if best_weights:
        ens_pred = sum(w * week_preds[mn]
                       for mn, w in best_weights.items())
        ens_metrics, _, _ = compute_all_metrics(week_true, ens_pred, tsc)
        week_preds["Ensemble"] = ens_pred

        weight_str = ", ".join(
            [f"{mn}={w:.1f}" for mn, w in best_weights.items()])
        print(f"  最优权重: {weight_str}")
        print(f"  集成 MAPE(150) = {ens_metrics['MAPE_150']:.4f}")
        print(f"  集成 PICP90 = {ens_metrics['PICP_90']:.3f} | "
              f"R² = {ens_metrics['R2']:.4f}")

        if "Ensemble" in all_results:
            all_results["Ensemble"]["week"] = ens_metrics
        else:
    ...
    for wi in plot_indices:
        if "Ensemble" in week_preds:
            plot_week_probabilistic(
                os.path.join(root, f"week_Ensemble_{wi}.png"),
                week_true[wi], week_preds["Ensemble"][wi], "Ensemble", wi, tsc
            )
            plot_day_probabilistic(
                os.path.join(root, f"day1_Ensemble_{wi}.png"),
                week_true[wi, :SPD], week_preds["Ensemble"][wi, :SPD],
                "Ensemble", wi, tsc
            )
        ...
        if "Ensemble" in week_preds:
            p50 = tsc.inverse_transform(
                week_preds["Ensemble"][wi, :, qi50].reshape(-1, 1)).flatten()
            ax.plot(hours, p50, label='Ensemble',
                    color=COLORS["Ensemble"], lw=1.5, ls='--')
        for d in range(1, 7):
            ax.axvline(d * 24, color='gray', ls='--', alpha=0.3)
        ax.set_title(f'Ensemble周预测 (样本#{wi})', fontsize=14)
        ax.set_xlabel('小时')
        ax.set_ylabel('电价 (元/MWh)')
    ...
    # 保存模型权重和 scaler 用于实时推理
    import pickle
    # 保存各子模型权重
    for model_name, model in trained_models.items():
        torch.save(model.state_dict(), os.path.join(root, f"model_weights_{model_name}.pt"))
    # 保存 scaler 和配置
    ...
    ensemble_config = {
        ...
        "quantiles": QUANTILES,
        "sub_models": list(trained_models.keys()),
    }
    if best_weights:
        ensemble_config["ensemble_weights"] = {k: float(v) for k, v in best_weights.items()}
    with open(os.path.join(root, "model_config.json"), "w") as f:
        json.dump(ensemble_config, f, indent=2, ensure_ascii=False)
    print(f"  模型权重和scaler已保存到: {root}")

    save_results = {}
    if "Ensemble" in all_results:
        for scope, metrics in all_results["Ensemble"].items():
            if isinstance(metrics, dict):
                key = f"Ensemble_{scope}"
                save_results[key] = {
    ...
    if best_weights:
        run_config["ensemble_weights"] = {
            k: float(v) for k, v in best_weights.items()
        }
    ...
    if "Ensemble" in all_results and "direct" in all_results["Ensemble"]:
        r = all_results["Ensemble"]["direct"]
        print(f"  {'Ensemble':12s} | {r['MAPE_150']:>10.4f} | "
    ...
    print(f"  {'模型':12s} | {'MAPE(150)':>10s} | {'PICP90':>6s} | "
          f"{'PICP95':>6s} | {'PINAW90':>7s} | {'R²':>6s}")
    print(f"  {'-'*12}-+-{'-'*10}-+-{'-'*6}-+-{'-'*6}-+-{'-'*7}-+-{'-'*6}")
    if "Ensemble" in all_results and "week" in all_results["Ensemble"]:
        r = all_results["Ensemble"]["week"]
        print(f"  {'Ensemble':12s} | {r['MAPE_150']:>10.4f} | "
              f"{r['PICP_90']:>6.3f} | {r['PICP_95']:>6.3f} | "
              f"{r['PINAW_90']:>7.4f} | {r['R2']:>6.4f}")
